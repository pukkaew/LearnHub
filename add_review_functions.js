...
// Add review functions after updateReviewsDisplay function
const searchPattern = /\}\)\.join\(''\);\s*\}\s*async function loadRelatedCourses/;
const match = content.match(searchPattern);

if (match) {
    const reviewFunctions = `}).join('');
}
...
// Show review form for enrolled users
...
async function loadRelatedCourses`;
    ...
    content = content.replace(searchPattern, reviewFunctions);
    fs.writeFileSync(filePath, content, 'utf8');
    console.log('✅ Added review functions');
} else {
    console.log('❌ Could not find the pattern to insert functions');
    // Debug: show what we're looking for
    if (content.includes('async function loadRelatedCourses')) {
        console.log('Found loadRelatedCourses function');
    }
}
