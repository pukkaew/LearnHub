...
async function verifyCourse103() {
    try {
        const pool = await poolPromise;

        console.log('\n' + '='.repeat(70));
        console.log('🔍 กำลังตรวจสอบ Course ID: 103 (FINAL VERIFICATION)...');
        ...
        if (result.recordset.length === 0) {
            console.log('❌ ไม่พบ Course ID 103');
            process.exit(1);
        }

        const course = result.recordset[0];

        console.log('📋 ข้อมูล Course ID: 103');
        console.log('='.repeat(70));
        ...
        // Check duration is 2.50
        const durationValue = parseFloat(course.duration_hours);
        const durationCheck = durationValue === 2.5 || durationValue === 2.50;
        ...
        if (durationCheck && titleCheck && maxStudentsCheck) {
            ...
            if (!durationCheck) console.log(`   - duration_hours ไม่ใช่ 2.5 (ได้: ${course.duration_hours})`);
